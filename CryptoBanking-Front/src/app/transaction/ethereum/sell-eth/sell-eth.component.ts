...
import { TransactionEsell } from 'src/app/shared/transactionE.model';

@Component({
  selector: 'app-sell-eth',
  templateUrl: './sell-eth.component.html',
  styles: [
  ]
})
export class SellEthComponent implements OnInit {
  ...
  onSubmitE(form: NgForm) {
    if(confirm('Da li siguran da želiš izvršiti transakciju?')){
      this.service.postTransactionEp().subscribe(
      res => {
        this.resetForm(form);
  ...
  resetForm(form: NgForm) {
    form.form.reset();
    this.service.formDataEp = new TransactionEsell();
  }
}
