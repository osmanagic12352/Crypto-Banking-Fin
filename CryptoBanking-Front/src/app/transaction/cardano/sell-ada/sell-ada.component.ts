...
import { TransactionAsell } from 'src/app/shared/transactionA.model';

@Component({
  selector: 'app-sell-ada',
  templateUrl: './sell-ada.component.html',
  styles: [
  ]
})
export class SellAdaComponent implements OnInit {
  result: any;
  coinPrice: any = [];
  cardDetails: any;
  ...
  onSubmitA(form: NgForm) {
    if(confirm('Da li siguran da želiš izvršiti transakciju?')){
      this.service.postTransactionAp().subscribe(
      res => {
        this.resetForm(form);
  ...
  resetForm(form: NgForm) {
    form.form.reset();
    this.service.formDataAp = new TransactionAsell();
  }
}
